            ...
            if (Login::checkPermission($login, 'can_manage_inventory') && Login::notDemoMode($login)) {
                try {
                    ...
                    $save = array(
                        ...
                        'category' => $request->post('write_category_text')->topic(),
                    ...
                    if ($index) {
                        $metas = array();
                        foreach (Language::get('INVENTORY_METAS', array()) as $key => $label) {
                            $metas[$key] = \Inventory\Category\Model::save($key, $request->post($key.'_text')->topic());
                        }
                        // ตาราง inventory
                        ...
                        if ($index->id == 0) {
                            ...
                            if (empty($save['product_no'])) {
                                ...
                                $save['product_no'] = \Inventory\Number\Model::get($index->id, 'product_no', $table_items, 'product_no');
                            } else {
                        ...
                        if (empty($ret)) {
                            ...
                            if ($index->id == 0) {
                                ...
                                $inventory_id = \Inventory\Product\Model::create($save);
                            } else {
                                // แก้ไขรายละเอียดของสินค้าเท่านั้น
                                $inventory_id = \Inventory\Product\Model::update($index, $save);
                            }
                            if (!empty($metas)) {
                                // อัปเดต meta
                                $db->delete($table_meta, array(
                                    array('inventory_id', $inventory_id),
                                    array('name', array_keys($metas)),
                                ), 0);
